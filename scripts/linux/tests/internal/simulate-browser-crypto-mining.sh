...
#!/usr/bin/env bash
# Script: scripts/linux/tests/internal/simulate-browser-crypto-mining.sh
# Purpose: simulate the first network signal of a browser/host performing
#          unauthorized cryptocurrency mining ("cryptojacking") — a DNS
#          lookup of a known mining pool domain — to confirm Suricata's
#          DNS reputation/threat-intel rules fire and packetdevil reacts.
...
DEFAULT_DOMAIN="xmr.2miners.com"
...
Looks up a known cryptocurrency mining pool domain, simulating the DNS
lookup a browser tab or host compromised with cryptojacking malware would
make right before connecting out to mine on an attacker's behalf — this is
the kind of query a DNS reputation/threat-intel Suricata rule is meant to
catch. Querying an explicit public resolver (${DEFAULT_RESOLVER} by
default) bypasses any local DNS cache so the lookup actually goes out over
the WAN every time.

${DEFAULT_DOMAIN} is a real, legitimate Monero mining pool's domain. This
script only performs a DNS lookup against it — no mining software runs, no
connection to the pool is made, and no CPU/electricity is consumed beyond
the lookup itself. It is used here purely as a known-bad-reputation domain
...
Options:
  --domain <domain>   Override the domain looked up (advanced use).
...
echo "About to look up known mining-pool domain ${domain} via ${resolver}."
